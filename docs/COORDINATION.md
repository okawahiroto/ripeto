...
1. アプリアイコン・スプラッシュ画面（ブランドカラー決定が先決）
2. プライバシーポリシー・利用規約
3. App Store 申請準備（スクリーンショット・説明文）
4. EAS Submit 設定（`eas.json` の `ascAppId` / `appleTeamId` 追記）
5. `REVENUECAT_API_KEY_ANDROID` の設定（Google Play Console 登録後）
