...
2. App Store 申請準備（スクリーンショット・説明文）
   - ~~説明文・キーワード原稿~~ ✅ → docs/STORE_LISTING.md
   - ~~EAS Metadata (store.config.json)~~ ✅
   - ~~スクリーンショット撮影~~ ✅
   - ~~プライバシーポリシー URL ホスティング~~ ✅ → https://okawahiroto.github.io/ripeto/privacy.html
3. EAS Submit 設定（`eas.json` の `ascAppId` / `appleTeamId` 追記）← Apple Developer 登録後
