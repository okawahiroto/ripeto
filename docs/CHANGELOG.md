...
### デザイン
- コンセプト: 五線譜 + ト音記号 + 繰り返し記号（終わり側）
- カラー: 背景 `#F8F7F2`（楽譜の紙色）/ アクセント `#1C2B4A`（モーツァルトインク色・深紺）
- Figma で作成（AI 画像生成を参考に）

### 対応内容
- `assets/images/icon.png` — iOS 用（1024×1024、アルファなし）
- `assets/images/adaptive-icon.png` — Android アダプティブアイコン（1024×1024）
- `assets/images/splash-icon.png` — スプラッシュロゴ（1024×1024、透明背景）
- `icon.png` のアルファチャンネルを `#F8F7F2` で塗りつぶし（iOS 審査要件）

### 動作確認
- iOS シミュレータ: アイコン・起動・動作 ✅（EAS preview ビルド）
- iOS 実機: アイコン・起動・動作 ✅（EAS preview-device ビルド）

---

## 2026-05-01 — Android 動作確認（新アイコン）

### 実施内容
- `eas build --profile preview --platform android` で新アイコン入り APK ビルド
- エミュレータ・実機の両方でアイコン・起動・動作確認 ✅
